...
import type { Recordable } from '@edp-core/typings';

import type { FormActions, FormSchema, EDPFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@edp-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
...
function getDefaultState(): EDPFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<EDPFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | EDPFormProps = null;
  stateHandler: StateHandler;

  public store: Store<EDPFormProps>;
  ...
  private prevState: null | EDPFormProps = null;

  constructor(options: EDPFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<EDPFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: EDPFormProps) => Partial<EDPFormProps>)
      | Partial<EDPFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<EDPForm /> is not mounted');
    }
    return this.form;
  }
