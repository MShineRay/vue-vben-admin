...
import type { ValueType, EDPButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@edp-core/icons';
import { cn, isFunction } from '@edp-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { EDPRenderContent } from '../render-content';
import EDPButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<EDPButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <EDPRenderContent :content="btn.label" />
      </slot>
    </Button>
  </EDPButtonGroup>
</template>
<style lang="scss" scoped>
.edp-check-button-group {
  display: flex;
  flex-wrap: wrap;
